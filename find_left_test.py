...
# 在物体比较远，几乎没有偏移的情况下，如res_findleft中的visible0.jpg、thermal0.jpg，合适的left值约为155
# 在物体比较近，如res_findleft中的visible1.jpg、thermal1.jpg，合适的left值约为155
def find_left(visible, thermal, sigma_visible, sigma_thermal):
    relation = np.zeros(250)
    thermal = thermal[0:240, 60:300]
    H_p = frequency.H_gauss_LPF(thermal.shape, sigma_thermal, double_pad=True)
    H_p = 1 - H_p
    ...
    for i in range(125, 225):
        ...
        visible_tmp = cv2.resize(visible_tmp, (240, 240), interpolation=cv2.INTER_CUBIC)
        H_p = frequency.H_gauss_LPF(visible_tmp.shape, sigma_visible, double_pad=True)
        H_p = 1 - H_p
...
length = 768
left = find_left(visible_image, thermal_image, 30, 30)
top = 0
right = left + length
